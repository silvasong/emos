...
public class MenuServiceImpl implements MenuService {
	...
	public void saveMenu(Tmenu menu) {
		...
		if (menu.getPid() != 0
			...
			throw new EmosException("error.MenuServiceImpl.saveMenu.pid");
		}
		menuDao.create(menu);
	}
	...
	public void updateMenu(Tmenu menu) {
		...
		if (menu.getPid() != 0
				&& menuDao.get(menu.getPid()) == null) {
			throw new EmosException("error.MenuServiceImpl.updateMenu.pid");
		}
		if(menu.getMenuId() == menu.getPid()){
			throw new EmosException("error.MenuServiceImpl.updateMenu.other.pid");
		}
		menuDao.update(menu);
	}
